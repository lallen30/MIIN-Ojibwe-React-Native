...
export class NavigationMonitorService {
  ...
  static async checkForUpdates() {
    try {
      ...
      try {
        // Use the endpoint directly without concatenating with BASE_URL
        // since axiosRequest already has baseURL set to API.BASE_URL
        const termsResponse = await axiosRequest.get(API.ENDPOINTS.GET_TERMS_DATE);
      ...
      try {
        // Use the endpoint directly without concatenating with BASE_URL
        // since axiosRequest already has baseURL set to API.BASE_URL
        const privacyResponse = await axiosRequest.get(API.ENDPOINTS.GET_PRIVACY_DATE);
  ...
  static async getTermsPublishedDate() {
    try {
      // Use the endpoint directly without concatenating with BASE_URL
      const response = await axiosRequest.get(API.ENDPOINTS.GET_TERMS_DATE);
  ...
  static async getPrivacyPublishedDate() {
    try {
      // Use the endpoint directly without concatenating with BASE_URL
      const response = await axiosRequest.get(API.ENDPOINTS.GET_PRIVACY_DATE);
      return response.data.date;
    } catch (error) {
